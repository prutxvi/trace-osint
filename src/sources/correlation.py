...
def _cross_link_entities(entity_map: dict, findings: list[Finding]):
    """Cross-link entities that co-occur in the same findings.

    For each finding, all entities derived from that finding are linked together.
    Link strength is scored by the number of shared findings: more shared sources
    means a stronger relationship. Links are bidirectional and deduplicated.
    """
    finding_entity_map = {}
    for entity in entity_map.values():
    ...
    link_scores: dict[tuple[str, str], int] = {}
    for fid, eids in finding_entity_map.items():
        for i in range(len(eids)):
            for j in range(i + 1, len(eids)):
                pair = tuple(sorted([eids[i], eids[j]]))
                link_scores[pair] = link_scores.get(pair, 0) + 1

    entity_by_id = {e.id: e for e in entity_map.values()}
    for (eid_a, eid_b), score in link_scores.items():
        if score < 1:
            continue
        if eid_a not in entity_by_id or eid_b not in entity_by_id:
            continue
        entity_a = entity_by_id[eid_a]
        entity_b = entity_by_id[eid_b]
        if eid_b not in entity_a.linked_entity_ids:
            entity_a.linked_entity_ids.append(eid_b)
        if eid_a not in entity_b.linked_entity_ids:
            entity_b.linked_entity_ids.append(eid_a)
