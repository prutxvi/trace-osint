...
def _describe_person(profile: CanonicalProfile) -> str:
    """Describe the primary target in plain language."""
    parts = []
    ...
    if not parts:
        return f"Primary candidate profile: {profile.display_name or 'Unknown'}"
    return " ".join(parts)
...
def _describe_risk(profile: CanonicalProfile, exposure: dict) -> str:
    """Describe risk level in plain language."""
    ...
    if "public_profile_found" in signals:
        signal_parts.append("public profiles on multiple platforms")
    if "email_exposed" in signals:
        signal_parts.append("email address publicly exposed")
    if "phone_exposed" in signals:
        signal_parts.append("phone number publicly exposed")
    if "breach_presence" in signals or "data_breach" in signals:
        signal_parts.append("appears in breach index")

    if signal_parts:
        signal_text = "with " + ", ".join(signal_parts[:3])
    elif signals:
...
def _describe_verdict(profile: CanonicalProfile, exposure: dict) -> str:
    """Generate a clear, professional verdict sentence."""
    score = exposure.get("score", 0.0)
    risk_level = exposure.get("risk_level", "low")
    name = profile.display_name or "this target"

    if risk_level == "critical":
        return f"Critical exposure: {name} has an extensive public digital footprint requiring immediate attention."
    if risk_level == "high":
        return f"High exposure: {name} has a significant public footprint across multiple platforms and services."
    if risk_level == "medium":
        return f"Moderate exposure: {name} has a moderate public presence with some notable signals."
    if risk_level == "low":
        return f"Low exposure: {name} has minimal public footprint. This is a valid target with limited publicly available information."
    return f"Exposure level for {name} could not be determined from available data."
